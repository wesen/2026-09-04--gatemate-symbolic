...
module dataflow_top(input wire clk_10m,fpga_but,uart_rx_pin,output wire user_led,uart_tx_pin);
 ...
 dataflow_link link(.clk(clk_10m),.rst_n(rst_n),.rx(uart_rx_pin),.tx(uart_tx_pin),.idle(idle));
