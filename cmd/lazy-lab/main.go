...
import (
	...
	lazy "github.com/wesen/2026-09-04--gatemate-symbolic/pkg/lazy"
)
...
func NewCommand() *Command {
	return &Command{cmds.NewCommandDescription("lazy-lab", cmds.WithShort("Reset and verify an lazy graph example on the model or FPGA"), cmds.WithFlags(
		...
		fields.New("example", fields.TypeChoice, fields.WithChoices("shared", "cycle", "indirection", "indirection-cycle", "overflow"), fields.WithDefault("shared")),
...
func (c *Command) RunIntoGlazeProcessor(ctx context.Context, v *values.Values, p middlewares.Processor) error {
	...
	if settings.Engine == "serial" {
		s, err := lazy.NewSerial(settings.Device)
		if err != nil {
			return err
		}
		engine = s
	...
	outputs, snapshot, err := lazy.RunExample(ctx, engine, settings.Example)
	if err != nil {
		return err
	}
