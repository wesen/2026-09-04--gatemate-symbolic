...
assert '198 passed' in (OUT/'P5-final-tests.log').read_text()
...
coverage=json.loads((OUT/'P5-coverage.json').read_text())
assert len(coverage['faults'])==9
assert coverage['branches']['taken'] and coverage['branches']['not_taken']
print('198 tests; 9 fault kinds; both branch outcomes; four board captures; 11 printed slips verified')
